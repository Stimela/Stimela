function val = ozoncc_g(Option, myTagName,filename1,filename2)
...
case 'ExternData_Button_Load'
  ...
  if F ~= 0,
    ...
    ozoncc_g('ExternData_Load', myTagName);
  end;		% if F~= 0
...
case 'ExternData_Button_Plot'
  ...
  %	Plot external data
  ozoncc_g('ExternData_Plot', myTagName);
...
case 'SimResults_Button_Export'
  ...
  if F ~= 0,
    ...
    O3effl = ozonccEM(4*(NumCel+1)+1, :)';
    
    % ingoing measurements 
    %!![a,Variables] = varia;  
    V = st_Varia;
    eval(['load ' simfile(1:length(simfile)-4) '_in.sti -mat']);
    %!!coO3 = ozonccin(strmatch('Ozone ',Variables)+1,:)';				% ingoing DOC 
    coO3 = ozonccin(V.Ozone+1,:)'
...
%	  Pr9=eval(Pr9,'error(''Ongeldige Waarde 9'')');
   ...
   %   HeigthSPBC = Pr3;
      ...
      UV254BC   = ozonccEM(9*(NumCelTotBC+1)+2:10*(NumCelTotBC+1)+1,:);
      UV254BCce = UV254BC(:,TaantalBC); 
      LUV254BC  = UV254BCce(NumCelCumBC+1);
      OzoneBC   = ozonccEM(3*(NumCelTotBC+1)+2:4*(NumCelTotBC+1)+1,:);
      OzoneBCce = OzoneBC(:,TaantalBC);
      LOzoneBC  = OzoneBCce(NumCelCumBC+1);
      VLabBC    = hreactBC(NumCelCumBC+1);
      OzoneBCcg  = ozonccEM(4*(NumCelTotBC+1)+2:5*(NumCelTotBC+1)+1,TaantalBC);
      OzoneBCcgs = ozonccEM(5*(NumCelTotBC+1)+2:6*(NumCelTotBC+1)+1,TaantalBC);
      OzoneBCcgo = ozonccEM(4*(NumCelTotBC+1)+2,TaantalBC);
      OzoneBCcge = ozonccEM(5*(NumCelTotBC+1)+1,TaantalBC);
     ...
     db0BC= 3; %wordt opgegeven in ozoncc_s
...
pnt=[NaN NaN  1  NaN NaN  1   1   1  NaN  1  NaN  1  NaN NaN  1  NaN;...
  ...
  n = n+1;
  CallbackStr='ozoncc_g(''ExternData_Button_Load'')';
  ...
  n = n+1;
  CallbackStr='ozoncc_g(''ExternData_Button_Plot'')';
  ...
  n = n+1;
  CallbackStr='ozoncc_g(''ExternData_Button_Hide'')';
  ...
  n = n+1;
  CallbackStr='ozoncc_g(''ExternData_Button_Edit'')';
  ...
  n = n+1;
  CallbackStr='ozoncc_g(''ExternData_Button_New'')';
  ...
  y = 0.062;
  CallbackStr='ozoncc_g(''SimResults_Button_Export'')';
  UData.ExternData.NewButtHndl = uicontrol(...
    'Style','pushbutton',...
    'Units','Normalized', ...
...
grid on
  ...
  subplot(4,1,4)
%plot(TimeNumBC,ozonccEM);%      ozon in
%plot(TimeNum,Ct,'k')
%hold on
%plot(TimeNum,ozonccEM(3*(NumCelTot+1)+2,:),'g');%      ozon in
%hold on
%plot(TimeNum,ozonccEM(4*(NumCelTot+1)+1,:),'b');%      ozon uit
%hold on
%plot(TimeNum,ozonccEM(9*(NumCelTot+1)+2,:),'m');%      UV in
%hold on
%plot(TimeNum,ozonccEM(10*(NumCelTot+1)+1,:),'r');%     UV uit
%hold on
%plot(TimeNum,ozonccEM(0*(NumCelTot+1)+2,:),'g');%      ozon in
%hold on
%plot(TimeNum,ozonccEM(1*(NumCelTot+1)+2,:),'g');%      ozon in
%hold on
%plot(TimeNum,ozonccEM(2*(NumCelTot+1)+2,:),'g');%      ozon in
%hold on
%plot(TimeNum,ozonccEM(5*(NumCelTot+1)+2,:),'g');%      ozon in
%hold on
%plot(TimeNum,ozonccEM(6*(NumCelTot+1)+2,:),'g');%      ozon in
%hold on
%plot(TimeNum,ozonccEM(7*(NumCelTot+1)+2,:),'g');%      ozon in
%hold on
%plot(TimeNum,ozonccEM(8*(NumCelTot+1)+2,:),'g');%      ozon in
