function val = chloro_g(naamchloroffile,plotfile)
%standard graphic output file for chloro

if nargin<1
   ib = st_findblock(gcs,'chloro');
   if length(ib)==1
      naampelsoffile = get_pfil(ib{1});
   else
    % meerdere blokken   
     fs = get_pfil(ib);
     n = Radiobox(strvcat(ib),'Select the parameter file of type chloro to be visualized');
     if length(n)
       naampelsoffile = fs{n};
     else
       return;
...
Pa = st_getPdata(naampelsoffile, 'chloro');

NumCel = Pa.NumCel; %aantal cellen

eval(['load ' naampelsoffile(1:length(naamchlorofile)-4) '_in.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamchlorofile)-4) '_out.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamchlorofile)-4) '_EM.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamchlorofile)-4) '_ES.sti -mat']);
