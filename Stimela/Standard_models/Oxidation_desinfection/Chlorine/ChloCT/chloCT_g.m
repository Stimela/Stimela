function val = chloCT_g(naamchloCTffile,plotfile)
%standard graphic output file for chloCT

if nargin<1
   ib = st_findblock(gcs,'chloCT');
   if length(ib)==1
      naampelsoffile = get_pfil(ib{1});
   else
    % meerdere blokken   
     fs = get_pfil(ib);
     n = Radiobox(strvcat(ib),'Select the parameter file of type chloCT to be visualized');
     if length(n)
       naampelsoffile = fs{n};
     else
       return;
...
Pa = st_getPdata(naampelsoffile, 'chloCT');

NumCel = Pa.NumCel; %aantal cellen

eval(['load ' naampelsoffile(1:length(naamchloCTfile)-4) '_in.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamchloCTfile)-4) '_out.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamchloCTfile)-4) '_EM.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamchloCTfile)-4) '_ES.sti -mat']);
