...
void ozonox_Derivatives(real_T *sys, real_T t, real_T *xC, real_T *xD, real_T *uWQ, real_T *uES,
...
FactorKLO3   = *P("FactorKLO3");   // variable in the equation of Hughmark which varies for a single gas bubble (0.061)
...
/*
Numcel_All_n = *P("N_Numcel");
NumCel_All   = P("NumCel");      // Reeks van volledig gemengde vaten [-]

NumCel=0;
for (i=0;i<Numcel_All_n;i++)
{
  NumCel       += NumCel_All[i];		// Total number of completely mixed reactors [-]
}
...
/*
  ...
  hreact = flipud(cumsum(flipud(dh))-flipud(dh)/2); % Gaat ervan uit dat indien de bellenkolom uit meerdere blokken bestaat
                                                  % deze zijn opgegeven vanaf de ozondoseerschotel, zie opmerking ozonox_d
...
/*
if (FactorKLO3==99.0); // in case ozonox_s is called from ozoncc the FactorKLO3 is made 0 in ozonox_i and the kLO3 should be zero
{
    kLO3 = 0.0;	//kLO3 = zeros(NumCel,1);
}
else
{
    ...
    //kLO3 = kLO3.*KLO3Init; // defined for co-current IS INSERTED IN LOOP WHERE kLO3 IS USED
}
*/
kLO3 = (DifO3/db0)*(2.0+FactorKLO3*(pow(((pow((vb0*db0/nu),0.48))*(pow((nu/DifO3),0.34))*pow(((db0*pow(9.81,0.33))/pow(DifO3,0.66)),0.072)),1.61)));
...
extern void ozonox_Update(real_T t, real_T *xC, real_T *xD, real_T *uWQ, real_T *uES,
                         mxArray *Bin, mxArray *Uin, mxArray *Pin)
{
  //flag =2 vullen xD;
...
void ozonox_Outputs(real_T *sysWQ, real_T *sysEM, real_T t, real_T *xC, real_T *xD, real_T *uWQ, real_T *uES, 
                         mxArray *Bin, mxArray *Uin, mxArray *Pin)
{
...
double coO3Dos, coO3Dos_kUV, O3Dos_kUV, ceO3Dos_kUV;
