function [sys,x0,str,ts] = ozonox_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = ozonox_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with ozonox_p.m and defined in ozonox_d.m
% B =  Model size, filled with ozonox_i.m,
% x0 = initial state, filled with ozonox_i.m,
...
%   if FactorKLO3==99; % in case ozonox_s is called from ozoncc the FactorKLO3 is made 0 in ozonox_i and the kLO3 should be zero
%       kLO3 = zeros(NumCel,1);
%   else
...
%aanpassen dat hij alleen de x waarden van ozon en UV checkt
% if flag == 1 || flag==3, % Hernoemen x-vectoren
%   nx = x<0;
%   nx2 = x<-0.3/10000;
%   x(find(nx))=0;
% %   nxO3   = find(x(1:NumCel)<0);
% %   nxcgO3 = find(x(NumCel+1:2*NumCel)<0);
% %   nxUV   = find((x(2*NumCel+1:3*NumCel)-ceUVA254)<0);
% %   nx2 = [x(1:NumCel);x(NumCel+1:2*NumCel);(x(2*NumCel+1:3*NumCel)-ceUVA254)]<-0.3/10000;
% %  
% %   x(nxO3)=0;
% %   x(NumCel+nxcgO3)=0;
% %   x(2*NumCel+nxUV)=ceUVA254;
% end
