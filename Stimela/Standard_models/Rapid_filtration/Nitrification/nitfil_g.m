function val = nitfil_g(naamfilterfile,naamspoelfile)
...
if nargin<1
  ib = st_findblock(gcs,'nitfil');
  if length(ib)==1
    naamfilterfile = get_pfil(ib{1});
  else
    % meerdere blokken   
     fs = get_pfil(ib);
     n = Radiobox(strvcat(ib),'Select the parameter file of the nitrification filter to be visualized');
     if length(n)
       naamfilterfile = fs{n};
     else
       return;
...
P = st_getPdata(naamfilterfile, 'nitfil');
...
tijd   = nitfilEM(1,:)/(24*3600);
nh41   = nitfilEM(2,:);
nh4s1  = nitfilEM(NumCel+2,:);
nh4eff = nitfilEM(NumCel+1,:);
no21   = nitfilEM(2*NumCel+2,:);
no2s1  = nitfilEM(3*NumCel+2,:);
no2eff = nitfilEM(3*NumCel+1,:);
xns1   = nitfilEM(4*NumCel+2,:);
xnb1   = nitfilEM(5*NumCel+2,:);
po4eff = nitfilEM(14*NumCel+1,:);


%===edited code
[NoIn, LengteIn]  = size(nitfilin);
coNH4        = nitfilin(V.Ammonia+1,:);
coNO2        = nitfilin(V.Nitrite+1,:);
coO2         = nitfilin(V.Oxygen+1,:);
%===new code
Flush     = nitfilES(1+1, :);    % flush signal: 0 = filter running, 1 = back washing
%=============================STOP : AvdBerge, 09/09/03=========================================


%Lengteout = size(nitfilout,2);
ceNH4     = nitfilout(V.Ammonia+1,:);
ceNO2     = nitfilout(V.Nitrite+1,:);
ceO2      = nitfilout(V.Oxygen+1,:);
...
set(gcf,'Name',['Nitrification filter']);
stimMous(gcf);
