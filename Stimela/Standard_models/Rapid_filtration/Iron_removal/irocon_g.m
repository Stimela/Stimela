...
P = st_getPdata(naamfilterfile, 'irocon');
...
%Sigenk=[iroconEM(1,:);iroconEM(1+(NumCel+1):(2*NumCel+1),:)];
effenk=[iroconEM(1,:);iroconEM(1+1:(NumCel+1),:)];
efffe2=[iroconEM(1,:);iroconEM(1+(NumCel+1):(2*NumCel+1),:)];
...
%=============================START: AvdBerge, 09/09/03=========================================
%===edited code
[NoIn, LengteIn]  = size(iroconin);
Ql        = iroconin(V.Flow+1,LengteIn);
Tl        = iroconin(V.Temperature+1,LengteIn);
%coSS      = iroconin(V.Suspended_solids+1,LengteIn);
coSS      = iroconin(V.Iron3+1,LengteIn);
%===new code
Flush     = iroconin(NoIn, :);    % flush signal: 0 = filter running, 1 = back washing
%=============================STOP : AvdBerge, 09/09/03=========================================

eval(['load ' naamfilterfile(1:length(naamfilterfile)-4) '_out.sti -mat']);

LengteUit = size(iroconout,2);
%ceSS      = iroconout(V.Suspended_solids+1,LengteUit);
ceSS      = iroconout(V.Iron3+1,LengteUit);


%SSmax  = max(iroconout(V.Suspended_solids+1,:));
SSmax  = max(iroconout(V.Iron3+1,:));
