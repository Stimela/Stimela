function [sys,x0,str,ts] = wssetc_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = wssetc_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with wssetc_p.m and defined in wssetc_d.m
% B =  Model size, filled with wssetc_i.m,
% x0 = initial state, filled with wssetc_i.m,
...
if flag == 1, % Continuous states derivative calculation
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wssetc has complex or nan values in flag=1')
...
elseif flag ==2, %discrete state determination
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wssetc has complex or nan values in flag=2')
    sys((~isreal(sys))| isnan(sys))=0;
  end
...
elseif flag ==3, % output data determination
  ...
  sys(1)=u(1);
   if ~isnan(P.Maximal)
     if sys(1)>P.Maximal
       error([gcb ': value ' num2str(sys(1)) ' is larger than maximal value']);
       sys(1)=P.Maximal;
     end
   end
   ...
   if ~isnan(P.Minimal)
     if sys(1)<P.Minimal
       error([gcb ': value ' num2str(sys(1)) ' is smaller than minimal value']);
       sys(1)=P.Minimal;
     end
   end
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wssetc has complex or nan values in flag=3')
    sys((~isreal(sys))| isnan(sys))=0;
  end
