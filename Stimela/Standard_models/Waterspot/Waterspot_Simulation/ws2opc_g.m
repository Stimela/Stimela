function val = ws2opc_g(naamws2opcffile,plotfile)


if nargin<1
   ib = st_findblock(gcs,'ws2opc');
   if length(ib)==1
      naampelsoffile = get_pfil(ib{1});
   else
    % meerdere blokken   
     fs = get_pfil(ib);
     n = Radiobox(strvcat(ib),'Select the parameter file of type ws2opc to be visualized');
     if length(n)
       naampelsoffile = fs{n};
     else
       return;
...
Pa = st_getPdata(naampelsoffile, 'ws2opc');

NumCel = Pa.NumCel; %aantal cellen

eval(['load ' naampelsoffile(1:length(naamws2opcfile)-4) '_in.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamws2opcfile)-4) '_out.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamws2opcfile)-4) '_EM.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamws2opcfile)-4) '_ES.sti -mat']);
