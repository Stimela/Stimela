function [sys,x0,str,ts] = wssetq_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = wssetq_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with wssetq_p.m and defined in wssetq_d.m
% B =  Model size, filled with wssetq_i.m,
% x0 = initial state, filled with wssetq_i.m,
...
if flag == 1, % Continuous states derivative calculation
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wssetq has complex or nan values in flag=1')
...
elseif flag ==2, %discrete state determination
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wssetq has complex or nan values in flag=2')
    sys((~isreal(sys))| isnan(sys))=0;
  end
...
elseif flag ==3, % output data determination
  ...
  sys(P.uNumber) = u(U.Number+1);

  if ~isnan(P.Maximal)
    if sys(P.uNumber)>P.Maximal
      error([gcb ' value ' num2str(sys(P.uNumber)) ' is larger than maximal value']);
      sys(P.uNumber)=P.Maximal;
    end
  end

  if ~isnan(P.Minimal)
    if sys(P.uNumber)<P.Minimal
      error([gcb ' value ' num2str(sys(P.uNumber)) ' is smaller than minimal value']);
      sys(P.uNumber)=P.Minimal;
    end
  end
...
%  sys(P.uNumber)=x(1);
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wssetq has complex or nan values in flag=3')
    sys((~isreal(sys))| isnan(sys))=0;
  end
