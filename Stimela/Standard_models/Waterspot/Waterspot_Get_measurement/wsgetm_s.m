function [sys,x0,str,ts] = wsgetm_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = wsgetm_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with wsgetm_p.m and defined in wsgetm_d.m
% B =  Model size, filled with wsgetm_i.m,
% x0 = initial state, filled with wsgetm_i.m,
...
if flag == 1, % Continuous states derivative calculation
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wsgetm has complex or nan values in flag=1')
...
%   if (x(1)~=u(P.MeasurementNo))
%     assignin('base',P.WSTag,u(P.MeasurementNo));
%   end
% 
%   sys(1)=u(P.MeasurementNo);
  
  %%%% <= MODEL-SPECIFIC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  if any( (~isreal(sys)) | isnan(sys))
    disp('wsgetm has complex or nan values in flag=2')
    sys((~isreal(sys))| isnan(sys))=0;
  end
...
elseif flag ==3, % output data determination
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wsgetm has complex or nan values in flag=3')
    sys((~isreal(sys))| isnan(sys))=0;
  end
