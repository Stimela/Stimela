...
% u = input vector
%
% P =  proces parameters, filled with block0_p.m and defined in wsgetq_d.m
...
if flag == 1, % Continuous states derivative calculation
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wsgetq has complex or nan values in flag=1')
...
%   sys(1)=u(P.uNumber);
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wsgetq has complex or nan values in flag=2')
    sys((~isreal(sys))| isnan(sys))=0;
  end
...
elseif flag ==3, % output data determination
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('wsgetq has complex or nan values in flag=3')
    sys((~isreal(sys))| isnan(sys))=0;
  end
