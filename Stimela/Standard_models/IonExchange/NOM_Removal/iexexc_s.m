function [sys,x0,str,ts] = iexfil_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = iexfil_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with iexfil_p.m and defined in iexfil_d.m
% B =  Model size, filled with iexfil_i.m,
% x0 = initial state, filled with iexfil_i.m,
...
if any(abs(flag)==[1 2 3])
  ...
  coDOC   = u(U.DOC)-DOCna;  %Acetic acid    [mg/l]
...
if flag == 1, % Continuous states derivative calculation
  ...
  %nX = x(1:NumCel)<0.00000001;
  %x(nX)=0.0000001;
  %sys(1:NumCel)=((nX)|(sys(1:NumCel)>0)).*sys(1:NumCel);
  
  if Spoel==0
    sys(1:2*NumCel)=-(1/90)*x(1:2*NumCel); 
  else
   %sys(1:2*NumCel) = MatQ1*[coDOC;x(1:NumCel)] - M*[x(1:NumCel)-(x(NumCel+1:2*NumCel)/K).^(1/n);zeros(NumCel,1)]; % concentratie,accumulatie
   ...
   %sys(1:NumCel)=VelReal*MatQ2*[coDOC;x(1:NumCel)]-(1+FilPor)/FilPor*(rhoK*M)*[K*(sign(x(1:NumCel)).*(abs(x(1:NumCel))).^n)-x(NumCel+1:2*NumCel)];
   %sys(NumCel+1:2*NumCel)=vel/rhoK*MatQ2*[coDOC;x(1:NumCel)];
   sys(NumCel+1:2*NumCel)=M*[K*(sign(x(1:NumCel)).*(abs(x(1:NumCel))).^n)-x(NumCel+1:2*NumCel)];
   
   
   %sys(3*NumCel+1:4*NumCel)=nX'.*sys(3*NumCel+1:4*NumCel);
...
elseif flag ==3, % output data determination
  ...
  sys(U.DOC)=x(NumCel)+DOCna;
  
  % Determine extra measurements
  % eg. sys(U.Number+1) = x(1)/P.Opp;
  Spoel1=u(U.Number+1);  
  
  sys(U.Number+1:U.Number+(NumCel+2))= [coDOC+DOCna;x(1:NumCel)+DOCna;Spoel1];
