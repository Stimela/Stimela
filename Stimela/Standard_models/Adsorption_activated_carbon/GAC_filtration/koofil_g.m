function val = koofil_g(Option, myTagName, naamfilterfile, naamspoelfile, plotfile)
...
case 'ExternData_Button_Load'
  ...
  if Fn ~= 0,
    ...
    koofil_g('ExternData_Load', myTagName);
  end;		% if Fn~= 0
...
case 'ExternData_Button_Plot'
  ...
  %	Plot external data
  koofil_g('ExternData_Plot', myTagName);
...
case 'SimResults_Button_Export'
  ...
  P = st_getPdata(naamfilterfile, 'koofil');
...
case 'Fig_Draw'
  ...
  %	Retrieve variables
  LengthIn  = size(koofilin,2);																			% number of data points
  Ql        = koofilin(V.Flow+1,LengthIn);   		% ingoing flow
  Tl        = koofilin(V.Temperature+1,LengthIn);	% ingoing temperature
  coDOC     = koofilin(V.DOC+1,LengthIn);				% ingoing DOC
  
  
  %	Retrieve variables
  LengthOut = size(koofilout,2);																	% number of data points
  ceDOC     = koofilout(V.DOC+1,LengthOut);		% outgoing DOC
  SSmax  = max(koofilout(V.DOC+1,:));					% maximum value DOC during simulation run
 ...
 %    'PaperPosition', [0.25 0.25 10.5 8], 'Name', 'Activated carbon filtation');
  ...
  subplot(2,2,1)
  NormEff = koofilEM(NumCel+2,:)/coDOC;
  plot(Time,NormEff)
  ...
  TpsS=koofilEM(1,Cycle_Start:Cycle_Stop);
  
  
  SSig=koofilEM(2:NumCel+2,Cycle_Start:Cycle_Stop);
  grSig=size(SSig);
  eind=grSig(2);
  ...
  n = n+1;
  CallbackStr='koofil_g(''ExternData_Button_Load'')';
  ...
  n = n+1;
  CallbackStr='koofil_g(''ExternData_Button_Plot'')';
  ...
  n = n+1;
  CallbackStr='koofil_g(''ExternData_Button_Hide'')';
  ...
  n = n+1;
  CallbackStr='koofil_g(''ExternData_Button_Edit'')';
  ...
  n = n+1;
  CallbackStr='koofil_g(''ExternData_Button_New'')';
  ...
  y = 0.062;
  CallbackStr='koofil_g(''SimResults_Button_Export'')';
  UData.ExternData.NewButtHndl = uicontrol(...
    'Style','pushbutton',...
    'Units','Normalized', ...
