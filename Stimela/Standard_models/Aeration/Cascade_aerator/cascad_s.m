function [sys,x0,str,ts] = cascad_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = cascad_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with cascad_p.m and defined in cascad_d.m
% B =  Model size, filled with cascad_i.m,
% x0 = initial state, filled with cascad_i.m,
...
%   coCaCO3 = 0;%u(U.Calcium_carbonate);        %influent concentratie CaCO3 [mg/l]
  ...
  k2CH4    = P.k2CH4;   %de k2 van methaan [1/s]
  ...
  VBak     = P.VBak;   %de inhoud van een cascade bak
  Vair     = P.Vair;   % inhoud van de afgesloten lucht boven de cascade
  
  
  
%   PercFe   = P.PercFe;   %percentage ijzer dat hydroliseert in de cascade [%/100]
%   PercCa   = P.PercCa;   %percentage calcium dat kristalliseert in de cascade [%/100]
% 
%   PercSlib=0;
  ...
  %Td       = (NumCel*VBak)/Ql;   % Bepalen van de gemiddelde verblijftijd in de cascade
...
%   sys(NumCel+1)=MatQ6*[cgoO2;x(NumCel+1)]-((k2O2*kDO2)/RQeff)*x(NumCel+1)+(k2O2/RQeff)*x(1);
...
%   koolstofdioxidegas=sys(5*NumCel+1:6*NumCel);
  ...
  sys(8*NumCel+1:9*NumCel)=(cgoN2-x(8*NumCel+1:9*NumCel))./TijdStap-((k2N2*kDN2)/RQeff)*x(8*NumCel+1:9*NumCel)+(k2N2/RQeff)*x(7*NumCel+1:8*NumCel);
  ...
  sys(10*NumCel+1:11*NumCel)=(cgoH2S-x(10*NumCel+1:11*NumCel))./TijdStap-((k2H2S*kDH2S)/RQeff)*x(10*NumCel+1:11*NumCel)+(k2H2S/RQeff)*x(9*NumCel+1:10*NumCel);
  ...
  sys(1:NumCel)=MatQ1*[coO2;x(1:NumCel)]+k2O2*kDO2*x(NumCel+1:2*NumCel)-k2O2*x(1:NumCel); %-0.25*MatQ5*[pFe;x(8*NumCel+1:9*NumCel)];
%   sys(NumCel+1)=MatQ6*[cgoO2;x(NumCel+1)]-((k2O2*kDO2)/RQeff)*x(NumCel+1)+(k2O2/RQeff)*x(1);
  sys(NumCel+1:2*NumCel)=(x(11*NumCel+1)-x(NumCel+1:2*NumCel))./TijdStap-((k2O2*kDO2)/RQeff)*x(NumCel+1:2*NumCel)+(k2O2/RQeff)*x(1:NumCel);
...
%   koolstofdioxidegas=sys(5*NumCel+1:6*NumCel);
  ...
  sys(7*NumCel+1:8*NumCel)=MatQ1*[coN2;x(7*NumCel+1:8*NumCel)]+k2N2*kDN2*x(8*NumCel+1:9*NumCel)-k2N2*x(7*NumCel+1:8*NumCel);
  sys(8*NumCel+1:9*NumCel)=(x(11*NumCel+4)-x(8*NumCel+1:9*NumCel))./TijdStap-((k2N2*kDN2)/RQeff)*x(8*NumCel+1:9*NumCel)+(k2N2/RQeff)*x(7*NumCel+1:8*NumCel);

  
  sys(9*NumCel+1:10*NumCel)=MatQ1*[coH2S;x(9*NumCel+1:10*NumCel)]+k2H2S*kDH2S*x(10*NumCel+1:11*NumCel)-k2H2S*x(9*NumCel+1:10*NumCel);
  sys(10*NumCel+1:11*NumCel)=(x(11*NumCel+5)-x(10*NumCel+1:11*NumCel))./TijdStap-((k2H2S*kDH2S)/RQeff)*x(10*NumCel+1:11*NumCel)+(k2H2S/RQeff)*x(9*NumCel+1:10*NumCel);
