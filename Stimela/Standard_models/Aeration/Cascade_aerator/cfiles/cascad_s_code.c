/*
This is the C-version of the Stimela cascade model.
 ...
 *
 */
void cascad_Derivatives(real_T *sys, real_T t, real_T *xC, real_T *xD, real_T *u,
                         mxArray *Bin, mxArray *Uin, mxArray *Pin)

{
double  Tl,Ql,QgTot,coO2,coCH4,coCO2,coHCO3;
double  pHo,EGVo,coN2, coH2S, coMn;
int NumCel;
double k2CH4, RQeff, VBak, Vair;
double Tg, TijdStap, IonStrength, f;
double K1,K2,Kw, KS;
int i;
...
//double pFe[20];
 ...
 QgTot    = u[U("Number")+1]/3600.0; //total airflow [m3/h]
  ...
  k2CH4    = *P("k2CH4");   //the k2-value of methane [1/s]
  RQeff    = *P("RQeff");   //the effective air-water ratio
  VBak     = *P("VBak");   //volume of cascade tank
  ...
  if (Vair>0)
  {  
      dTransportGas(dgoO2, goO2, xcgoO2[0], 1/TijdStap, NumCel);
  }
  else
  {
      dTransportGas(dgoO2, goO2, cgoO2, 1/TijdStap, NumCel);
  }
  dReactionX(dgoO2, goO2, -1*k2O2*kDO2/RQeff, NumCel);
  dReactionX(dgoO2, O2, k2O2/RQeff, NumCel);
  ...
  if(Vair>0)
  {
  dTransportGas(dgoCH4,goCH4,xcgoCH4[0],1/TijdStap,NumCel);
  }
  else
  {
  dTransportGas(dgoCH4,goCH4,cgoCH4,1/TijdStap,NumCel);
  }   
  dReactionX(dgoCH4,goCH4,-1*k2CH4*kDCH4/RQeff,NumCel);
  dReactionX(dgoCH4,CH4, k2CH4/RQeff, NumCel);
  ...
  if (Vair>0)
  {
   dTransportGas(dgoCO2, goCO2, xcgoCO2[0], 1/TijdStap, NumCel);
  }
  else
  {
      dTransportGas(dgoCO2, goCO2, cgoCO2, 1/TijdStap, NumCel);
  }
  dReactionX(dgoCO2, goCO2, -1*k2CO2*kDCO2/RQeff, NumCel);
  dReactionX(dgoCO2, CO2, k2O2/RQeff, NumCel);
  ...
  if(Vair>0)
  {
  dTransportGas(dgoN2, goN2, xcgoN2[0], 1/TijdStap, NumCel);
  }
  else
  {
      dTransportGas(dgoN2, goN2, cgoN2, 1/TijdStap, NumCel);
  }
  dReactionX(dgoN2, goN2, -1*k2N2*kDN2/RQeff, NumCel);
  dReactionX(dgoN2, N2, k2N2/RQeff, NumCel);
  ...
  if (Vair>0)
  {
  dTransportGas(dgoH2S, goH2S, xcgoH2S[0], 1/TijdStap, NumCel);
  }
  else
  {
      dTransportGas(dgoH2S, goH2S, cgoH2S, 1/TijdStap, NumCel);
  }
  dReactionX(dgoH2S, goH2S, -1*k2H2S*kDH2S/RQeff, NumCel);
  dReactionX(dgoH2S, H2S, k2H2S/RQeff, NumCel);
  
  if (Vair>0)
  {
   
   *dcgoO2=(cgoO2*QgTot -(O2[NumCel-1]-coO2)*Ql - xcgoO2[0]*QgTot)/Vair; // O2 in gas
   *dcgoCH4=(cgoCH4*QgTot -(CH4[NumCel-1]-coCH4)*Ql - xcgoCH4[0]*QgTot)/Vair; // CH4 in gas
   *dcgoCO2=(cgoCO2*QgTot -(CO2[NumCel-1]-coCO2)*Ql - xcgoCO2[0]*QgTot)/Vair; // CO2 in gas
   *dcgoN2=(cgoN2*QgTot -(N2[NumCel-1]-coN2)*Ql - xcgoN2[0]*QgTot)/Vair; // N2 in gas
   *dcgoH2S=(cgoH2S*QgTot -(H2S[NumCel-1]-coH2S)*Ql - xcgoH2S[0]*QgTot)/Vair; // H2S in gas
 ...
 *
 */
 void cascad_Update(creal_T t, real_T *xC, real_T *xD, real_T *u,
                         mxArray *Bin, mxArray *Uin, mxArray *Pin)
{
  //flag =2 vullen xD;
 ...
 */
 ...
 void cascad_Outputs(real_T *sys, real_T t, real_T *xC, real_T *xD, real_T *u,
                         mxArray *Bin, mxArray *Uin, mxArray *Pin)
{
double  Tl,Ql,coO2,coCH4,coCO2,coHCO3;
double  pHo,EGVo,coMn, coN2, coH2S, CO2eff, Mneff;
int NumCel;
double k2CH4, RQeff, VBak;
double Tg,  TijdStap, IonStrength, f;
double K1,K2,Kw, KS;
int i;
