function [sys,x0,str,ts] = vacslp_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = vacslp_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with vacslp_p.m and defined in vacslp_d.m
% B =  Model size, filled with vacslp_i.m,
% x0 = initial state, filled with vacslp_i.m,
...
if any(abs(flag)==[1 2 3])
  ...
  QgSleep  = P.QgDrag;  %sleep luchtdebiet [m3/s]
  QgVers   = P.QgFresh;  %verse luchtdebiet [m3/s]
  QgRec    = P.QgRec;  %recirculatie luchtdebiet [m3/s]
  PercFe   = P.PercFe;     %percentage ijzer dat hydroliseert in de vacuumontgasser [%/100]
  Hoogte   = P.Height;     %hoogte van het gepakte bed [m]
  Diam     = P.Diam;     %diameter van het gepakte bed [m]
  PercCa   = P.PercCa;     %percentage calcium dat kristalliseert in de vacuumontgasser [%/100]
  PercSlib = 0;       %percentage ijzerslib dat achterblijft in de vacuumontgasser [%/100]

  % eerst element uit Uv is altijd de breedte van waterstroom (NumElem)
...
%%%%%%%%
  ...
  %Aangepaste gasconcentraties in de vacuumontgasser  
  cgoO2  = (QgSleep*cgoO2) /QgTot;
  cgoCH4 = (QgSleep*cgoCH4)/QgTot;
  cgoCO2 = (QgSleep*cgoCO2)/QgTot;
  cgoN2  = (QgSleep*cgoN2) /QgTot;
  cgoH2S = (QgSleep*cgoH2S)/QgTot;
...
if flag == 1, % Continuous states derivative calculation
  ...
  % nummer de toestande van 1..n, n moet opgegeven worden in vacslp_i

  %GELIJKSTROOM De lucht is volledig gemengd (1 bakje)
  ...
  sys(5*NumCel+5)=MatQ6*[cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);x(5*NumCel+5)]+[(sum(deel(4*NumCel+1:5*NumCel)))];  %gas N2
...
elseif flag ==3, % output data determination
  ...
  % eg. sys(U.Temparature) = x(1);
   ...
   sys(U.Bicarbonate)=x(7*NumCel+5)*MrHCO3;
   sys(U.pH)=pK1-log10(x(3*NumCel))+log10(x(7*NumCel+5)*fi(1,4,EGVo));
   sys(U.Iron2)=(coFe2*(1-PercFe)+x(8*NumCel+5))*MrFe;
   sys(U.Iron3)=x(9*NumCel+5)*MrFe;
   sys(U.Calcium)=(coCa2*(1-PercCa)+x(10*NumCel+5))*MrCa;
  ...
  % eg. sys(U.Number+1) = x(1)/P.Opp;
   ...
   sys(U.Number+1+(NumCel+1):U.Number+2*(NumCel+1))=MrO2*[(cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+1)];
   sys(U.Number+1+2*(NumCel+1):U.Number+3*(NumCel+1))=MrO2*kDO2*[(cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+1)];
   sys(U.Number+1+3*(NumCel+1):U.Number+4*(NumCel+1))=MrCH4*[coCH4;x(1*NumCel+1:2*NumCel)];
   sys(U.Number+1+4*(NumCel+1):U.Number+5*(NumCel+1))=MrCH4*[(cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+2)];
   sys(U.Number+1+5*(NumCel+1):U.Number+6*(NumCel+1))=MrCH4*kDCH4*[(cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+2)];
   sys(U.Number+1+6*(NumCel+1):U.Number+7*(NumCel+1))=MrCO2*[coCO2;x(2*NumCel+1:3*NumCel)];
   sys(U.Number+1+7*(NumCel+1):U.Number+8*(NumCel+1))=MrCO2*[(cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+3)];
   sys(U.Number+1+8*(NumCel+1):U.Number+9*(NumCel+1))=MrCO2*kDCO2*[(cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+3)];
   sys(U.Number+1+9*(NumCel+1):U.Number+10*(NumCel+1))=MrN2*[coN2;x(3*NumCel+1:4*NumCel)];
   sys(U.Number+1+10*(NumCel+1):U.Number+11*(NumCel+1))=MrN2*[(cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+4)];
   sys(U.Number+1+11*(NumCel+1):U.Number+12*(NumCel+1))=MrN2*kDN2*[(cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+4)];
   sys(U.Number+1+12*(NumCel+1):U.Number+13*(NumCel+1))=MrH2S*[coH2S;x(4*NumCel+1:5*NumCel)];
   sys(U.Number+1+13*(NumCel+1):U.Number+14*(NumCel+1))=MrH2S*[(cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+5)];
   sys(U.Number+1+14*(NumCel+1):U.Number+15*(NumCel+1))=MrH2S*kDH2S*[(cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot));ones(NumCel,1)*x(5*NumCel+5)];

   %Verblijftijd voor C- en F-functies
   %sys(U.Number+1+15*(NumCel+1):U.Number+16*(NumCel+1))=[cStap;x(5*NumCel+6:6*NumCel+5)];
   sys(U.Number+1+15*(NumCel+1):U.Number+16*(NumCel+1))=MatQ3*[cStap;x(5*NumCel+6:6*NumCel+5);x(6*NumCel+5)];
   ...
   % partiele drukken
   pO2(1:NumCel+1,1)  = R*(Tl+273)*1000*[cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+1)];
   pCH4(1:NumCel+1,1) = R*(Tl+273)*1000*[cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+2)];
   pCO2(1:NumCel+1,1) = R*(Tl+273)*1000*[cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+3)];
   pN2(1:NumCel+1,1)  = R*(Tl+273)*1000*[cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+4)];
   pH2S(1:NumCel+1,1) = R*(Tl+273)*1000*[cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+5)];
   pH2O(1:NumCel+1,1) = pw(Tg+1)*ones(NumCel+1,1);
   PTot(1:NumCel+1,1) = pO2+pCH4+pCO2+pN2+pH2S+pH2O;

   %Partiele drukken O2, CH4, CO2, N2, H2S,H2O
   sys(U.Number+1+23*(NumCel+1):U.Number+24*(NumCel+1))=pO2;  
   sys(U.Number+1+24*(NumCel+1):U.Number+25*(NumCel+1))=pCH4; 
   sys(U.Number+1+25*(NumCel+1):U.Number+26*(NumCel+1))=pCO2; 
   sys(U.Number+1+26*(NumCel+1):U.Number+27*(NumCel+1))=pN2;  
   sys(U.Number+1+27*(NumCel+1):U.Number+28*(NumCel+1))=pH2S;  
   sys(U.Number+1+28*(NumCel+1):U.Number+29*(NumCel+1))=pH2O; 

   %Totale druk
   sys(U.Number+1+29*(NumCel+1):U.Number+30*(NumCel+1))=PTot; 

   %Volume fracties (NATTE LUCHT)
   sys(U.Number+1+30*(NumCel+1):U.Number+31*(NumCel+1))=([cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+1)]./([cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+1)]+[cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+2)]+[cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+3)]+[cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+4)]+[cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+5)])).*(1-pH2O./PTot);
   sys(U.Number+1+31*(NumCel+1):U.Number+32*(NumCel+1))=([cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+2)]./([cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+1)]+[cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+2)]+[cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+3)]+[cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+4)]+[cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+5)])).*(1-pH2O./PTot);
   sys(U.Number+1+32*(NumCel+1):U.Number+33*(NumCel+1))=([cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+3)]./([cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+1)]+[cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+2)]+[cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+3)]+[cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+4)]+[cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+5)])).*(1-pH2O./PTot);
   sys(U.Number+1+33*(NumCel+1):U.Number+34*(NumCel+1))=([cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+4)]./([cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+1)]+[cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+2)]+[cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+3)]+[cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+4)]+[cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+5)])).*(1-pH2O./PTot);
   sys(U.Number+1+34*(NumCel+1):U.Number+35*(NumCel+1))=([cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+5)]./([cgoO2*(QgVers/QgTot)+x(5*NumCel+1)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+1)]+[cgoCH4*(QgVers/QgTot)+x(5*NumCel+2)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+2)]+[cgoCO2*(QgVers/QgTot)+x(5*NumCel+3)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+3)]+[cgoN2*(QgVers/QgTot)+x(5*NumCel+4)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+4)]+[cgoH2S*(QgVers/QgTot)+x(5*NumCel+5)*(QgRec/QgTot);ones(NumCel,1)*x(5*NumCel+5)])).*(1-pH2O./PTot);
   sys(U.Number+1+35*(NumCel+1):U.Number+36*(NumCel+1))=pH2O./PTot;
