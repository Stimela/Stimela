...
if stage ==1 & length(zb),
  ...
  if OK,
    ...
    if all(vi == [0 90]) & OK,
      ...
      set(fg,'windowbuttondownfcn',['Dhv_delp(2,''' s2 ''')' dwn]);
        qm = Gcq;
...
elseif stage ==2 & length(zb),
  ...
  mot = get(fg,'windowbuttonmotionfcn');
  set(fg,'windowbuttonmotionfcn',['Dhv_delp(3)' mot])
  set(fg,'windowbuttonupfcn',['Dhv_delp(4,''' P1 ''')'])
...
%
      ...
      for tel = 1:length(h),
        if strcmp(get(h(tel),'type'),'line'),
          ...
          yplek = find(y(plek)<=limy(2) & y(plek)>=limy(1));
          if (length(yplek)),
            x(plek(yplek)) = [];
            y(plek(yplek)) = [];

            set(h(tel),'xdata',x,'ydata',y);
          end;
        end;
      end; %for loop
    end;
