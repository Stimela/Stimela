function val = dhvgrp_g(naamdhvgrpffile,plotfile)
%standard graphic output file for dhvgrp

if nargin<1
   ib = st_findblock(gcs,'dhvgrp');
   if length(ib)==1
      naampelsoffile = get_pfil(ib{1});
   else
    % meerdere blokken   
     fs = get_pfil(ib);
     n = Radiobox(strvcat(ib),'Select the parameter file of type dhvgrp to be visualized');
     if length(n)
       naampelsoffile = fs{n};
     else
       return;
...
Pa = st_getPdata(naampelsoffile, 'dhvgrp');

NumCel = Pa.NumCel; %aantal cellen

eval(['load ' naampelsoffile(1:length(naamdhvgrpfile)-4) '_in.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamdhvgrpfile)-4) '_out.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamdhvgrpfile)-4) '_EM.sti -mat']);
eval(['load ' naampelsoffile(1:length(naamdhvgrpfile)-4) '_ES.sti -mat']);
