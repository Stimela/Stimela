function [sys,x0,str,ts] = block0_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = block0_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with block0_p.m and defined in modmes_d.m
% B =  Model size, filled with block0_i.m,
% x0 = initial state, filled with block0_i.m,
...
if flag == 1, % Continuous states derivative calculation
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('modmes has complex or nan values in flag=1')
...
%   if (x(1)~=u(P.uNumber))
%       assignin('base',P.WSTag,u(P.uNumber))
%   end
%   
%   sys(1)=u(P.uNumber);
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('modmes has complex or nan values in flag=2')
    sys((~isreal(sys))| isnan(sys))=0;
  end
...
elseif flag ==3, % output data determination
  ...
  % default equal to the input with zeros for extra modmesements
  ...
  sys=u(P.MeasurementNo);

  % Determine extra modmesements
  % eg. sys(U.Number+1) = x(1)/P.Opp;
 
  %%%% <= MODEL-SPECIFIC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  if any( (~isreal(sys)) | isnan(sys))
    disp('modmes has complex or nan values in flag=3')
    sys((~isreal(sys))| isnan(sys))=0;
  end
