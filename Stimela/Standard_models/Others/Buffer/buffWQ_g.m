function val = buffWQ_g(naambuffWQffile,plotfile)


if nargin<1
   ib = st_findblock(gcs,'buffWQ');
   if length(ib)==1
      naampelsoffile = get_pfil(ib{1});
   else
    % meerdere blokken   
     fs = get_pfil(ib);
     n = Radiobox(strvcat(ib),'Select the parameter file of type buffWQ to be visualized');
     if length(n)
       naampelsoffile = fs{n};
     else
       return;
...
Pa = st_getPdata(naampelsoffile, 'buffWQ');

NumCel = Pa.NumCel; %aantal cellen

eval(['load ' naampelsoffile(1:length(naambuffWQfile)-4) '_in.sti -mat']);
eval(['load ' naampelsoffile(1:length(naambuffWQfile)-4) '_out.sti -mat']);
eval(['load ' naampelsoffile(1:length(naambuffWQfile)-4) '_EM.sti -mat']);
eval(['load ' naampelsoffile(1:length(naambuffWQfile)-4) '_ES.sti -mat']);
