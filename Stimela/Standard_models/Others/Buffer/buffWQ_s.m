function [sys,x0,str,ts] = buffWQ_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = buffWQ_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with buffWQ_p.m and defined in buffWQ_d.m
% B =  Model size, filled with buffWQ_i.m,
% x0 = initial state, filled with buffWQ_i.m,
...
if any(abs(flag)==[1 2 3])
  ...
  M3act = x(U.Flow); % on flow spot is the M3
...
if flag == 1, % Continuous states derivative calculation
  ...
  % concentrations
  if M3act>0
    sys = Fin/3600*u(1:U.Number) - Fout/3600*x(1:U.Number)/M3act;
  else
    error([gcb ' Buffer Level below 0']);
  end
  % Buffer level
  sys(U.Flow) = (Fin - Fout)/3600;
  
  if (sys(U.Turbidity)~=0)
%    disp(sys(U.Turbidity));
  end
  %%%% <= MODEL-SPECIFIC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  if any( (~isreal(sys)) | isnan(sys))
    disp('buffWQ has complex or nan values in flag=1')
...
elseif flag ==2, %discrete state determination
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('buffWQ has complex or nan values in flag=2')
    sys((~isreal(sys))| isnan(sys))=0;
  end
...
elseif flag ==3, % output data determination
  ...
  %Height
  sys(U.Number+1) = x(U.Flow)/P.Surface;
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('buffWQ has complex or nan values in flag=3')
    sys((~isreal(sys))| isnan(sys))=0;
  end
