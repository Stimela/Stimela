function [sys,x0,str,ts] = changq_s(t,x,u,flag,B,x0,U,P),
%  [sys,x0,str,ts] = changq_s(t,x,u,flag,B,x0,U,P),
%    Stimela S-Function
%
% t = time
...
% u = input vector
%
% P =  proces parameters, filled with changq_p.m and defined in changq_d.m
% B =  Model size, filled with changq_i.m,
% x0 = initial state, filled with changq_i.m,
...
if flag == 1, % Continuous states derivative calculation
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('changq has complex or nan values in flag=1')
...
elseif flag ==2, %discrete state determination
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('changq has complex or nan values in flag=2')
    sys((~isreal(sys))| isnan(sys))=0;
  end
...
%  sys(P.uNumber)=x(1);
  ...
  if any( (~isreal(sys)) | isnan(sys))
    disp('changq has complex or nan values in flag=3')
    sys((~isreal(sys))| isnan(sys))=0;
  end
